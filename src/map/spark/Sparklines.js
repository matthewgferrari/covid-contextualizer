...
class Sparklines extends PureComponent {
    ...
    render() {
        ...
        const {arr: points, yOfZero: refValue} = dataToPoints({ data, limit, width, height, margin, max, min });
        ...
        return (
            <svg {...svgOpts} aria-label="panel" >
                {
                    React.Children.map(this.props.children, function(child) {
                        return React.cloneElement(child, { data, points, width, height, margin, refValue });
                    })
                }
            </svg>
        );
