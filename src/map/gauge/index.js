...
const GaugeChart = (props) => {
  ...
  const initChart = (update, resize = false, prevProps) => {
    ...
    g.current = svg.current.append("g").attr("aria-label", "panel"); //Used for margins
    doughnut.current = g.current.append("g").attr("class", "doughnut").attr("aria-label", "panel");
    ...
    needle.current = g.current.append("g").attr("class", "needle").attr("aria-label", "panel");
...
const renderChart = (
  ...
  svg.current
    ...
    .attr(
      "height",
      //Extra 10 is for padding of bottom text
      height.current + margin.current.top + margin.current.bottom + 21
    ).attr("aria-label", "panel");
  g.current.attr(
    "transform",
  ...
  var arcPaths = doughnut.current
    ...
    .enter()
    .append("g")
    .attr("class", "arc").attr("aria-label", "panel");
  arcPaths
    .append("path")
...
const drawNeedle = (
  ...
  needle.current.append("path").attr("d", pathStr).attr("fill", needleColor).attr("aria-label", "panel");
  ...
  needle.current
    ...
    .attr("r", needleRadius)
    .attr("fill", needleBaseColor).attr("aria-label", "panel");
  if (!hideText) {
      ...
      .tween("progress", function () {
        ...
        return function (percentOfPercent) {
          ...
          return container.current
            .select(`.needle path`)
            .attr("d", calculateRotation(progress, outerRadius, width)).attr("aria-label", "panel");
        };
      });
  } else {
    container.current
      .select(`.needle path`)
      .attr("d", calculateRotation(percent, outerRadius, width)).attr("aria-label", "panel");
  }
};
...
const addText = (percentage, props, outerRadius, width, g) => {
  ...
  g.current
    .append("g")
    .attr("class", "text-group").attr("aria-label", "panel")
    .attr(
      "transform",
    ...
    .style("text-anchor", "middle").attr("aria-label", "panel");
};
